            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Lab Patients</h4>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="patients_data" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                     ...
                                     $patient_fetch = "SELECT * FROM patients WHERE collection_type='Lab' AND act='2' ORDER BY id DESC";
                                    ...
                                    while ($row = mysqli_fetch_array($fetch_query)) {    
                                       ...
                                       $trf=$row['trf_form'];
                                        ...
                                        $collection_type = $row['collection_type'];
                                         $branch=$row['branch'];
                                        $query2 = "SELECT * FROM taiyo_branches WHERE branch_id = '$branch'";
                                    $result2 = mysqli_query($conn, $query2);
                                    
                                    // Check if the query was successful and return the agent's name
                                    if ($result2) {
                                    $branch = mysqli_fetch_assoc($result2); // Fetch the first result (assuming there's only one)
                                    $branch_name = $branch['branch_area']; // Store the agent's name in a variable
