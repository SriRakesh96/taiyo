...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    /* Pass Hash */ 
    $psw_act = "SELECT * FROM taiyo_branchlogins WHERE email='$email' AND act='1'";
    ...
    if ($verify) {
        $login_pro = "SELECT * FROM taiyo_branchlogins WHERE email='$email' AND act='1'";
        ...
        if (mysqli_num_rows($login_conn) > 0) {
            ...
            setcookie('eng_email', $email, time() + 2678400, '/', '', false, true); // 86400 seconds = 24 hours
            setcookie('eng_password', $password, time() + 2678400, '/', '', false, true);
